...
nu_P = (c_star./m.J).*sum(nu_mat,1)'; % dim 1: j

% T_bar & lambda
T_Bar = m.T.^(1/m.theta)./(w.*m.tau);
MT_mat = repmat(M,[1,m.N]).*T_Bar.^(m.theta);
MT_sum = sum(MT_mat,1);
lambda_mat = MT_mat./repmat(MT_sum,[m.N,1]);

% update Dn
H_mat = func_hx(m,s_nj).*c_nj.^(m.theta-1);
D_new = D.*((MT_sum'.*c_star./m.J.*m.theta).*sum(H_mat,1)').^(1/(1+m.theta));
%%
% update w_i
prod_mat = (1-yita).*lambda_mat.*X;
prod = sum(prod_mat,2);
fmkt_cost = w.*m.F.*(c_star.^m.theta).*MT_sum';
net_prof_mat = yita.*lambda_mat.*X - w.*m.F.*(c_star.^m.theta).*MT_mat;
...
% update P_n
P_new = (P.^(-m.theta-1).*nu_P.*MT_sum').^(-1/m.theta);
